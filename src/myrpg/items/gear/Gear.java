...
public class Gear implements IGear {
    Map<GearSlots, IItem> gear = new HashMap<>();

    @Override
    public boolean isSlotBusy(GearSlots gearSlot) {
        return gear.containsKey(gearSlot);
    }

    @Override
    public IItem unequip(GearSlots gearSlot) {
    ...
    @Override
    public boolean equip(GearSlots gearSlot, IItem item) {
        if(gear.containsKey(gearSlot))
            return false;
